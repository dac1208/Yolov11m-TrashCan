...
# OUTPUT (novi dataset)
# ======================
OUT_BASE = SRC_BASE.parent / "yolo_format_cropmix_singleobj"
...
# CROP PARAMETRI (soft)
# ======================
CROP_SCALE = 1.5     # manje agresivno od 2.0
MIN_BOX_PX = 10      # izbjegni mikrocropove koji rade šum
SAVE_QUALITY = 95
...
def add_train_crops_singleobj():
    crop_count = 0
    skipped_tiny = 0
    skipped_not_singleobj = 0
    ...
    for img_path in SRC_TR_IMG.glob("*"):
        ...
        labels_all = read_yolo_labels(lbl_path)

        # KLJUC: cropamo samo slike s 1 objektom (nema unlabeled objekata u cropu)
        if len(labels_all) != 1:
            skipped_not_singleobj += 1
            continue

        cls, xc, yc, bw, bh = labels_all[0]
        ...
        out_stem = f"{img_path.stem}_crop"
    ...
    return crop_count, skipped_tiny, skipped_not_singleobj
...
def make_yaml():
    ...
    out_yaml = OUT_BASE / "trashcan_cropmix_singleobj.yaml"
...
if __name__ == "__main__":
    ...
    crops, skipped_tiny, skipped_not_singleobj = add_train_crops_singleobj()
    print(f"Added SINGLE-OBJ crop images to TRAIN: {crops}")
    print(f"Skipped tiny boxes (<{MIN_BOX_PX}px): {skipped_tiny}")
    print(f"Skipped non-singleobj train images: {skipped_not_singleobj}")
